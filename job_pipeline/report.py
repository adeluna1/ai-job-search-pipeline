...
def _card(record: dict[str, Any]) -> str:
    ...
    return f"""
    <article class="job-card {score_class}" data-score="{score:.1f}" data-fit="{_e(record['fit_label'])}" data-status="{_e(record.get('status', 'new'))}">
      <div class="card-head">
        <div>
          <p class="eyebrow">{_e(record['company'])}</p>
          <h2><a href="{_e(record['url'])}" target="_blank" rel="noreferrer">{_e(record['title'])}</a></h2>
...
def export_html(records: list[dict[str, Any]], path: Path, threshold: float) -> None:
...
<title>AI Job Match Shortlist</title>
<style>
...
<header class="hero"><h1>AI job match shortlist</h1><p>Evidence-based rankings derived from the configured resume profile. Verify every role on the employer's site before applying.</p><div class="stats"><div class="stat"><strong>{len(records)}</strong>ranked jobs</div><div class="stat"><strong>{strong_count}</strong>strong fits (≥ {threshold:g})</div><div class="stat"><strong>{companies}</strong>companies</div></div></header>
<div class="toolbar"><input id="search" type="search" placeholder="Filter title, company, or location"><select id="fit"><option value="">All fit levels</option><option>excellent</option><option>strong</option><option>possible</option><option>weak</option></select><select id="status"><option value="">All statuses</option><option>new</option><option>saved</option><option>applied</option><option>interviewing</option><option>offer</option><option>rejected</option><option>withdrawn</option></select></div>
...
const q=document.querySelector('#search'),fit=document.querySelector('#fit'),status=document.querySelector('#status');
function filterCards(){{const term=q.value.toLowerCase();document.querySelectorAll('.job-card').forEach(card=>{{const show=card.textContent.toLowerCase().includes(term)&&(!fit.value||card.dataset.fit===fit.value)&&(!status.value||card.dataset.status===status.value);card.hidden=!show}})}}
[q,fit,status].forEach(el=>el.addEventListener('input',filterCards));
</script></body></html>"""
...
def export_reports(records: list[dict[str, Any]], report_dir: Path, threshold: float, prefix: str = "job_matches") -> tuple[Path, Path]:
    ...
    export_html(records, html_path, threshold)
