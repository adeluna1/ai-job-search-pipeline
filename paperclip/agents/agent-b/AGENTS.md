...
- Reopen the employer-controlled posting and verify it is active on an exact ATS or employer-matching domain.
...
- Create Agent C work only from the integrity-bound handoff emitted for a live, direct-domain, unambiguously fresh `apply` decision.
...
- Missing or boundary-ambiguous dates and conflicting facts cannot create an Agent C handoff.
